...
noorlinkDatabase.getAllCountries = function() {
  return Object.entries(this.countryDatabase).map(([code, country]) => ({
    code,
    flag: country.flag,
    name: country.name,
    region: country.region,
    tier: country.tier,
    hasCustomPricing: country.customPricing || false
  }));
};

noorlinkDatabase.getCountriesByRegion = function(regionName) {
  return this.getAllCountries().filter(country =>
    country.region === regionName.toLowerCase().replace(' ', '-')
  );
};